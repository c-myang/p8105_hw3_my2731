...
stations between 1981 and 2010. It contains 2 595 176 observations of 7
variables. Key identifiers include the `id` of the station name and
`date`. The weather data includes the amount of precipitation (in tenths
of mm), snowfall (in mm), snow depth (mm), and max and min temperatures
(in tenths of C). We can note significant amounts of missing data:
...
data points are missing, especially for `tmax` and `tmin`, where almost
half of the observations have missing values.
...
year-round and therefore, most days of the year would observe 0
snowfall.
...
temp_jan = noaa_clean %>% 
  ...
  labs(
    ...
    title = "Average max temperature in January, 1981 to 2010",
    caption = "Data come from the noaa package"
  )
...
temp_jul = noaa_clean %>% 
  ...
  labs(
    x = "Year",
    y = "Mean max temperature (°C)",
    title = "Average max temperature in July, 1981 to 2010",
    caption = "Data come from the noaa package"
  )
...
![](p8105_hw3_my2731_files/figure-gfm/tmax_plots-1.png)<!-- -->
...
``` r
tmax_tmin_p = noaa_clean %>% 
  ggplot(aes(x = tmin, y = tmax)) + 
  geom_hex() + 
  labs(
    x = "Minimum temperature (°C)",
    y = "Max temperature (°C)",
    title = "Hexagon density plot of daily minimum and maximum temperature (°C) in New York, 1981-2010",
    caption = "Data come from the noaa package"
  ) + 
  viridis::scale_fill_viridis(name = "", option = "plasma")

snowfall_p = noaa_clean %>% 
  ...
  labs(
    x = "Year",
    y = "Snowfall (mm)",
    title = "Distribution of snowfall in New York, 1981 to 2010",
    caption = "Data come from the noaa package"
  )
...
![](p8105_hw3_my2731_files/figure-gfm/tmaxmin_plots-1.png)<!-- -->
